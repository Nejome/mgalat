...
<head>
    ...
    <meta name="author" content="@if(isset($settings->author) && $settings->author != '') {{$settings->author}} @endif">
    <meta name="description" content="@if(isset($settings->description) && $settings->description != '') {{$settings->description}} @endif">
    <meta name="keywords" content="@if(isset($settings->keywords) && $settings->keywords != '') {{$settings->keywords}} @endif">
    ...
    <nav class="navbar navbar-top navbar-horizontal navbar-expand-md navbar-dark">
        <div class="container px-4">
            <a class="navbar-brand" href="{{url('/admin')}}">
                <span style="color: white; font-size: 30px;">مجالات</span>
            </a>
        </div>
    </nav>
                ...
                <div class="card bg-secondary shadow border-0">
                    <div class="card-header bg-transparent pb-2">
                        ...
                        @if(session()->has('wrong_data'))
                            <div class="alert alert-danger">{{session()->get('wrong_data')}}</div>
                        @endif

                        <div class="text-muted text-center mt-2"><h2>تسجيل الدخول</h2></div>
